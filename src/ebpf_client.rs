...
#[cfg(target_os = "linux")]
use aya::{include_bytes_aligned, maps::HashMap as BpfHashMap, programs::TracePoint, Bpf, BpfLoader};
   ...
   pub fn new() -> Self {
      #[cfg(target_os = "linux")]
      {
         ...
         match Self::load_ebpf_program() {
            Ok(bpf) => EbpfClientIdentifier { bpf_program: Some(bpf), fallback_enabled: true },
            Err(_) => EbpfClientIdentifier { bpf_program: None, fallback_enabled: true },
         }
      }

      #[cfg(not(target_os = "linux"))]
      {
         EbpfClientIdentifier { fallback_enabled: true }
      }
   }
