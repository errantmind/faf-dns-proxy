...
pub trait Resolve {
   fn resolve_query(&self, query: &[u8]) -> Result<Vec<u8>, String>;

   /// Returns a list of IP addresses for the given server name. IP addresses may be IPv4 or IPv6.
   fn resolve_ips(&self, server_name_as_bytes: &[u8]) -> Result<Vec<std::net::IpAddr>, String>;
}
...
pub struct Resolver {
   config: crate::resolver_config::ResolverConfig,
   primary_request_tx_channel: kanal::AsyncSender<Vec<u8>>,
   primary_request_rx_channel: kanal::AsyncReceiver<Vec<u8>>,
   upstream_request_tx_channels: Vec<kanal::AsyncSender<Vec<u8>>>,
   upstream_response_rx_channels: Vec<kanal::AsyncReceiver<Vec<u8>>>,
}

impl Resolver {
   pub fn new(config: crate::resolver_config::ResolverConfig) -> Self {
      let mut upstream_request_tx_channels = Vec::with_capacity(config.get_servers().len());
      let mut upstream_response_rx_channels = Vec::with_capacity(config.get_servers().len());

      for server in config.get_servers() {
         let (request_tx, request_rx) = kanal::bounded_async(config.get_max_queue_depth());
         let (response_tx, response_rx) = kanal::bounded_async(config.get_max_queue_depth());
         tokio::task::spawn(Self::upstream_dns_resolver(request_rx, response_tx, *server, config.get_min_ttl_override()));
         upstream_request_tx_channels.push(request_tx);
         upstream_response_rx_channels.push(response_rx);
      }

      let (primary_request_tx_channel, primary_request_rx_channel) = kanal::bounded_async(config.get_max_queue_depth());
      tokio::task::spawn(Self::coordinator_loop(primary_request_tx_channel, primary_request_rx_channel));

      Self { config, primary_request_tx_channel, primary_request_rx_channel, upstream_request_tx_channels, upstream_response_rx_channels }
   }
   ...
   async fn coordinator_loop(
      ...
      loop {
         tokio::select! {
            query_msg = primary_request_rx_channel.recv() => {
               match query_msg {
                  Ok(data) => {
                     let upstream_request_tx_channel = &mut upstream_request_tx_channels[upstream_index];
                     upstream_request_tx_channel.send(data).await.unwrap();
                     upstream_index = (upstream_index + 1) % upstream_request_tx_channels.len();
                  },
                  Err(_) => panic!("Primary request channel has been closed prematurely")
               }
            }
         }
   ...
   async fn upstream_dns_resolver(
      request_rx: kanal::AsyncReceiver<Vec<u8>>,
      response_tx: kanal::AsyncSender<Vec<u8>>,
      ...
      let mut tls_stream = Self::connect(&tls_connector, &dns_server).await;
      let mut tls_stream_split = tokio::io::split(tls_stream);
      let mut write_handle = tokio::task::spawn(Self::handle_writes(tls_stream_split.1, request_rx, None, shutdown_writer_rx));
      let mut read_handle =
         tokio::task::spawn(Self::handle_reads(tls_stream_split.0, dns_server, min_ttl_override_maybe, response_tx, shutdown_reader_rx));
      ...
      loop {
         ...
         tls_stream = Self::connect(&tls_connector, &dns_server).await;
         tls_stream_split = tokio::io::split(tls_stream);
         write_handle = tokio::task::spawn(Self::handle_writes(tls_stream_split.1, msg_rx, unsent_query, shutdown_writer_rx));
         read_handle =
            tokio::task::spawn(Self::handle_reads(tls_stream_split.0, dns_server, min_ttl_override_maybe, response_tx, shutdown_reader_rx));
      }
   }
            ...
            match tls_connector.connect(tokio_rustls::rustls::ServerName::try_from(upstream_dns.server_name).unwrap(), tcp_stream).await {
               ...
               Err(err) => {
                  ...
                  if tls_failures > 1 && crate::util::is_power_of_2(tls_failures) {
                     eprintln!(
                        "failed {tls_failures}x times establishing tls connection to: {} with error: {}",
                        upstream_dns.socket_addr, err
                     );
                  }
                  tokio::time::sleep(tokio::time::Duration::from_millis(CONN_ERROR_SLEEP_MS)).await;
                  continue;
   ...
   async fn handle_writes(
      ...
      if let Some(unsent_previous_query) = unsent_previous_query_maybe {
         if let Some(unsent_data) = Self::write(&mut write_half, unsent_previous_query).await {
      ...
      loop {
         tokio::select! {
            ...
            query_msg = msg_rx.recv() => {
               match query_msg {
                  Ok(data) => match Self::write(&mut write_half, data).await {
                     Some(unsent_data) => {
                        // Qeury failed
                        return (msg_rx, Some(unsent_data))
                     },
      ...
      loop {
         {
            ...
            loop {
               ...
               {
                  {
                     // Keep id here to minimize race conditions
                     ...
                     if !cache_guard.contains_key(cache_key) {
                        {
                           ...
                           let wrote = response_tx.send(udp_segment_no_tcp_prefix, &saved_addr).await.unwrap();
