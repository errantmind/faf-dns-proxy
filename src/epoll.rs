...
fn threaded_worker(port: u16, cpu_core: i32) {
   ...
   let mut buf_upstream_response: [u8; REQ_BUFF_SIZE] = unsafe { core::mem::zeroed() };
   let _buf_upstream_response_start_address = &buf_upstream_response as *const _ as isize;
      ...
      for index in 0..num_incoming_events {
         ...
         if cur_fd == fd_client_udp_listener {
            ...
            if likely(read_client > 0) {
               println!("core: {}", cpu_core);
               // Extract just the id from the client request
               let id = unsafe { *(buf_client_request.as_mut_ptr() as *mut u16) };
               ...
               match tls_client_conn.write_tls(&mut sock) {
                  ...
                  Err(error) => println!("write_tls failed with error {}", error),
               }
            }
            ...
            if io_state.plaintext_bytes_to_read() > 0 {
               ...
               tls_client_conn.reader().read_exact(exact_slice).unwrap();
               let upstream_response_vec_slice = &mut exact_slice[2..];

               let id = unsafe { *(upstream_response_vec_slice.as_mut_ptr() as *mut u16) };
